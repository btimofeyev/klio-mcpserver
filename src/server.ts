  ...
  switch (method) {
    ...
    case 'tools/call':
      ...
      if (toolName === 'get_next_homework') {
        const result = await getNextHomework(
          toolArgs.child_id,
          toolArgs.subject
        );
      ...
      if (toolName === 'get_subject_context') {
        const result = await getSubjectContext(
          toolArgs.child_id,
          toolArgs.subject_name
        );
  ...
  try {
    ...
    if (searchType === 'review') {
      ...
      if (reviewMaterials.length > 0) {
        ...
        reviewMaterials.forEach((material: any) => {
    ...
    if (searchType === 'current_lesson') {
      ...
      if (currentLessons.length > 0) {
        ...
        currentLessons.forEach((lesson: any) => {
    ...
    if (searchType === 'performance_review' || searchType === 'all') {
      ...
      if (lowScoreItems.length > 0) {
        ...
        lowScoreItems.forEach((item: any) => {
          ...
          let suggestion = '';
          if (percentage < 50) suggestion = ' - Needs significant review';
          else if (percentage < 70) suggestion = ' - Could use more practice';
          else suggestion = ' - Room for improvement';
...
async function findNextUpMaterials(childSubjectIds: string[]) {
  try {
...
async function findLowPerformanceItems(childSubjectIds: string[]) {
  try {
    const { data, error } = await supabase
      .from('materials')
      .select(`
    ...
    const lowPerformanceItems = (data || []).filter((item: any) => {
...
async function getMaterialContent(childId: string, materialIdentifier: string): Promise<string> {
  try {
    ...
    const subjectName = (materials.child_subject as any)?.subject?.name || 
                       (materials.child_subject as any)?.custom_subject_name_override || 'General';
    ...
    if (materials.lesson_json) {
      ...
      if (content) {
        if (content.learning_objectives && content.learning_objectives.length > 0) {
          ...
          content.learning_objectives.forEach((obj: string) => {
            result.push(`- ${obj}`);
          });
          result.push('');
        }
    ...
    if (materials.parent_material) {
      result.push(`**Related Material:** ${(materials.parent_material as any).title}`);
      if ((materials.parent_material as any).lesson_json) {
        const parentContent = parseLessonContent((materials.parent_material as any).lesson_json);
...
// Get next homework assignment with full context
async function getNextHomework(childId: string, subject?: string): Promise<string> {
  try {
    ...
    if (subject) {
      const filteredIds = childSubjects
        .filter(cs => 
          ((cs.subject as any)?.name?.toLowerCase().includes(subject.toLowerCase())) ||
    ...
    const subjectName = (homework.child_subject as any)?.subject?.name || 
                       (homework.child_subject as any)?.custom_subject_name_override || 'General';
    ...
    if (homework.lesson_json) {
      ...
      if (content) {
        ...
        if (content.formatted_questions && content.formatted_questions.length > 0) {
          ...
          content.formatted_questions.slice(0, 3).forEach((question: string) => {
            result.push(`- ${question}`);
          });
          result.push('');
        }
    ...
    if ((homework.parent_material as any)?.lesson_json) {
      const parentContent = parseLessonContent((homework.parent_material as any).lesson_json);
      if (parentContent?.content_summary) {
        result.push(`**Related lesson:** ${(homework.parent_material as any).title}`);
...
async function getSubjectContext(childId: string, subjectName: string): Promise<string> {
  try {
    ...
    const matchingSubjects = childSubjects.filter(cs => 
      ((cs.subject as any)?.name?.toLowerCase().includes(subjectName.toLowerCase())) ||
      (cs.custom_subject_name_override?.toLowerCase().includes(subjectName.toLowerCase()))
    );

    if (matchingSubjects.length === 0) {
      const availableSubjects = childSubjects.map(cs => 
        (cs.subject as any)?.name || cs.custom_subject_name_override
      ).join(', ');
    ...
    const actualSubjectName = (matchingSubjects[0].subject as any)?.name || 
    ...
    if (graded.length > 0) {
      ...
      let totalEarned = 0;
      let totalPossible = 0;
      graded.slice(0, 5).forEach((item: any) => {
        const percentage = Math.round((item.grade_value / item.grade_max_value) * 100);
        const gradeEmoji = percentage >= 90 ? '🅰️' : percentage >= 80 ? '🅱️' : percentage >= 70 ? '🆔' : percentage >= 60 ? '🆘' : '❌';
...
async function getStudentProfile(childId: string): Promise<string> {
  try {
...
async function getNextHomeworkDetails(childSubjectIds: string[]): Promise<string | null> {
...
async function getSubjectMaterials(childSubjectIds: string[], query: string): Promise<string> {
...
async function getReviewMaterials(childSubjectIds: string[]) {
  const { data, error } = await supabase
    .from('materials')
    .select(`
...
async function findUpcomingAssessments(childSubjectIds: string[]) {
  const { data, error } = await supabase
    .from('materials')
    .select(`
...
async function findUpcomingAssessmentsBySubject(childSubjectIds: string[]) {
  return findUpcomingAssessments(childSubjectIds);
}

async function getCurrentLessons(childSubjectIds: string[]) {
  const { data, error } = await supabase
    .from('materials')
    .select(`
...
async function getCurrentLessonsBySubject(childSubjectIds: string[]) {
  return getCurrentLessons(childSubjectIds);
}

async function findGradedMaterialsBySubject(childSubjectIds: string[]) {
  const { data, error } = await supabase
    .from('materials')
    .select(`
      id, title, grade_value, grade_max_value, completed_at, content_type
