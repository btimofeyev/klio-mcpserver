...
export class QueryParser {
   ...
   */
  private static parseIntent(query: string): QueryIntent {
   ...
   */
  private static extractSubject(query: string): string | undefined {
   ...
   */
  private static extractContentType(query: string): QueryIntent['contentType'] {
    for (const [contentType, pattern] of Object.entries(this.CONTENT_TYPE_PATTERNS)) {
      if (pattern.test(query)) {
        return contentType as QueryIntent['contentType'];
      }
    }
    return undefined;
  }
   ...
   */
  private static extractUrgency(query: string): QueryIntent['urgency'] {
    for (const [urgency, pattern] of Object.entries(this.URGENCY_PATTERNS)) {
      if (pattern.test(query)) {
        return urgency as QueryIntent['urgency'];
      }
    }
    return undefined;
  }
   ...
   */
  private static extractStatus(query: string): QueryIntent['status'] {
   ...
   */
  static generateFilters(intent: QueryIntent) {
    const filters: any = {};
  ...
  static describeIntent(intent: QueryIntent): string {
    ...
    if (intent.urgency === 'overdue') parts.push('overdue');
    if (intent.status === 'incomplete') parts.push('incomplete');
    if (intent.subject) parts.push(intent.subject);
    if (intent.contentType) parts.push(intent.contentType);
