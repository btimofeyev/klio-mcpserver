...
import { Pool } from 'pg';
import { QueryIntent, QueryParser } from '../utils/queryParser.js';
import { MaterialData, ContentFormatter, SearchResult } from '../utils/formatter.js';
...
export class SearchService {
   ...
   */
  async intelligentSearch(childId: string, query: string): Promise<SearchResult[]> {
   ...
   */
  private getContentTypes(intent: QueryIntent): string[] {
   ...
   */
  private rankResults(materials: MaterialData[], intent: QueryIntent): MaterialData[] {
   ...
   */
  private calculateRelevanceScore(material: MaterialData, intent: QueryIntent): number {
    let score = 0;
   ...
   */
  private isOverdue(material: MaterialData): boolean {
    if (!material.due_date || material.completed_at) return false;
   ...
   */
  private isDueSoon(material: MaterialData): boolean {
    if (!material.due_date || material.completed_at) return false;
   ...
   */
  private isLowScore(material: MaterialData): boolean {
    if (!material.grade_value || !material.grade_max_value) return false;
   ...
   */
  async getMaterialById(childId: string, materialId: string): Promise<MaterialData | null> {
    ...
    return result.rows[0] as MaterialData;
  }

  /**
   * Get related materials (for enhanced context in fetch)
   */
  async getRelatedMaterials(material: MaterialData): Promise<MaterialData[]> {
