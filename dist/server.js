...
async function getChildSubjects(childId) {
    ...
    const result = await pool.query('SELECT id FROM child_subjects WHERE child_id = $1', [childId]);
 ...
 */
function formatEducationalContent(material) {
    const sections = [];
    // Basic info
    ...
    if (material.lesson_json) {
        try {
            ...
            if (lessonData.learning_objectives && lessonData.learning_objectives.length > 0) {
                ...
                lessonData.learning_objectives.forEach((obj) => sections.push(`• ${obj}`));
                sections.push('');
            }
            // Main content
...
function createMcpServer() {
    ...
    // ===============================
    mcpServer.tool('search', 'Search for educational materials including assignments, lessons, worksheets, quizzes, and tests', {
    ...
    // ===============================
    mcpServer.tool('fetch', 'Retrieve complete details and content for a specific educational material', {
